...
function mockValidate(returnCode = '1', userName = 'TestUser'): jest.Mock {
  return (fetchPostWithinPage as jest.Mock).mockResolvedValueOnce({
    isFound: true,
    value: { Header: { Status: '1' }, ValidateIdDataBean: { returnCode, userName } },
  });
}
...
function mockAccounts(cardNumber = '1234'): jest.Mock {
  return (fetchGetWithinPage as jest.Mock).mockResolvedValueOnce({
    isFound: true,
    value: {
      Header: { Status: '1' },
      DashboardMonthBean: {
...
function mockTxns(
  ...
  const txnGroups: { txnIsrael?: IScrapedTransaction[]; txnAbroad?: IScrapedTransaction[] } = {};
  ...
  return (fetchGetWithinPage as jest.Mock).mockResolvedValueOnce({
    ...
    value: {
      Header: { Status: '1' },
      CardsTransactionsListBean: { Index0: { CurrentCardTransactions: [txnGroups] } },
    },
  });
}
...
/**
 * Creates a fake IScrapedTransaction with randomized values.
 *
 * @param overrides - partial fields to override generated defaults
 * @returns a complete IScrapedTransaction
 */
