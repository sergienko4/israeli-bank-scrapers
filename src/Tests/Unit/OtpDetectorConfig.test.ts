// Regression tests for CR PR #286 findings F9 + F10 + F12. Validates the OTP
// detector config exports that have no behavior-dependent tests in
// OtpDetector*.test.ts:
...
//   • PHONE_PATTERN token boundaries (F12) — prevents partial matches inside
//     a longer masked value (e.g. ***12345 does NOT yield ***1234).
...
describe('OtpDetectorConfig — PHONE_PATTERN (CR PR #286 F10 + F12)', () => {
  it.each([
    ...
    // F12 — token boundaries (no partial matches inside longer masked values)
    ['***12345', false, 'F12: 5 trailing digits — partial match prevented'],
    ['***123456', false, 'F12: 6 trailing digits — partial match prevented'],
    ['1***1234', false, 'F12: leading digit context — partial match prevented'],
    ['****12345', false, 'F12: 4-star mask + 5 digits — partial match prevented'],
    // F12 — valid boundary contexts (non-digit/non-star delimiters)
    ['call ***1234 now', true, 'F12: space-delimited mask + 4 digits in sentence'],
    ['-***1234', true, 'F12: dash-prefixed mask is still valid'],
    ['***1234.', true, 'F12: trailing punctuation does not block match'],
  ])('PHONE_PATTERN.test(%j) === %s — %s', (input, expected) => {
    const didMatch = PHONE_PATTERN.test(input);
    expect(didMatch).toBe(expected);
  });

  it('extracts the canonical mask (not a truncated prefix) from longer text', () => {
    const text = 'Code sent to ******5100 — please enter it below';
    const result = PHONE_PATTERN.exec(text);
    expect(result?.[0]).toBe('******5100');
  });

  it('returns null (not a truncated prefix) when text has too many trailing digits (F12)', () => {
    const text = 'Reference: ***12345 is too long to be a 4-digit mask';
    const result = PHONE_PATTERN.exec(text);
