...
describe('login', () => {
  it('succeeds with valid credentials', async () => {
    ...
    const txnDetails = FIXTURES.mockCardTransactionDetails([txn]);
    setupFetchMocks(txnDetails);
...
describe('fetchData', () => {
  it('fetches and converts normal transactions', async () => {
    ...
    const txn = FIXTURES.scrapedTxn({
      ...
      trnTypeCode: TRN_TYPE_CODE.Regular,
    });
    ...
    expect(firstAccount?.accountNumber).toBe('4580');
    const txnResult = firstAccount?.txns[0];
    expect(txnResult?.originalAmount).toBe(-250);
    expect(txnResult?.type).toBe(TX_TYPES.Normal);
    expect(txnResult?.status).toBe(TX_STATUSES.Completed);
  });
  ...
  it('detects installment transactions', async () => {
    ...
    expect(txnResult?.type).toBe(TX_TYPES.Installments);
    expect(txnResult?.installments).toEqual({ number: 3, total: 12 });
  });

  it('handles credit transactions with positive amount', async () => {
    setupVisaCalMocks();
    const txn = FIXTURES.scrapedTxn({ trnTypeCode: TRN_TYPE_CODE.Credit, trnAmt: 50 });
    const details = FIXTURES.mockCardTransactionDetails([txn]);
    setupFetchMocks(details);
  ...
  it('throws on failed month data fetch', async () => {
    setupVisaCalMocks();
    const failedResponse = FIXTURES.mockCardTransactionDetails();
    failedResponse.statusCode = 0;
    failedResponse.title = 'Error';
    setupFetchMocks(failedResponse);
  ...
  it('calls filterOldTransactions when enabled', async () => {
    ...
    const details = FIXTURES.mockCardTransactionDetails([txn]);
    setupFetchMocks(details);
    await new VISA_CAL_SCRAPER(visaCalOptions()).scrape(FIXTURES.CREDS);
    expect(FILTER_OLD_TXNS).toHaveBeenCalled();
  });
  ...
  it('skips filtering when isFilterByDateEnabled is false', async () => {
    ...
    const details = FIXTURES.mockCardTransactionDetails([txn]);
    setupFetchMocks(details);
    const opts = visaCalOptions({ outputData: { isFilterByDateEnabled: false } });
    const result = await new VISA_CAL_SCRAPER(opts).scrape(FIXTURES.CREDS);
    expect(result.success).toBe(true);
    expect(FILTER_OLD_TXNS).not.toHaveBeenCalled();
  });
