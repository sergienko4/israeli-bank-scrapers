...
describe('NetworkDiscovery', () => {
  ...
  describe('buildBalanceUrl', () => {
    it('builds balance URL with account ID from captured pattern', async () => {
      await simulateResponse({
        url: 'https://bank.co.il/gatewayAPI/accountDetails/infoAndBalance/[REDACTED-OPER-ACCT]',
        body: { AccountBalance: 19856 },
      });
      const balUrl = discovery.buildBalanceUrl('[REDACTED-OPER-ACCT]');
      expect(balUrl).not.toBe(false);
      if (balUrl) {
        expect(balUrl).toContain('infoAndBalance/[REDACTED-OPER-ACCT]');
      }
    });
