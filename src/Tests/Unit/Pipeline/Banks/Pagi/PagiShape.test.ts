/**
 * Pagi (FIBI group) hard-model scrape shape — unit coverage for the
...
import PAGI_SHAPE from '../../../../../Scrapers/Pipeline/Banks/Pagi/scrape/PagiShape.js';
import {
  accountNumberOf,
  customerUrl,
  extractAccounts,
...
import {
  ...
  txnsUrl,
  txnsVars,
} from '../../../../../Scrapers/Pipeline/Banks/Pagi/scrape/PagiShapeTxns.js';
import type {
  ApiBody,
...
const ACCT: IPagiAcct = { accountNumber: '555001', branch: '770', accountType: 105 };
...
describe('PagiShape balance + identity urls', () => {
  ...
  it('customerUrl targets userData with a fresh uid on each call', () => {
    ...
    expect(first).toContain(`${PAGI_API}${USER_DATA_PATH}?uid=`);
    expect(first).not.toBe(second);
  });

  it('secondaryUrl targets the session accountType lookup', () => {
    const url = secondaryUrl();
    expect(url).toContain(`${PAGI_API}${BFF_BASE}/accountType?uid=`);
  });
...
describe('PagiShape transactions', () => {
  it('txnsUrl is the static BFF list endpoint (params ride the body)', () => {
    const url = txnsUrl();
    expect(url).toBe(`${PAGI_API}${BFF_BASE}/list`);
  });
...
describe('PAGI_SHAPE wiring', () => {
  it('declares GET accounts (with secondary identity) + GET balance + POST transactions', () => {
    expect(PAGI_SHAPE.customer.method).toBe('GET');
    expect(PAGI_SHAPE.customer.secondaryUrlTag).toBeDefined();
    expect(PAGI_SHAPE.balance.method).toBe('GET');
    expect(PAGI_SHAPE.transactions.method).toBe('POST');
  });
  ...
  it('declares no prime nav — AUTH-DISCOVERY handles the appsng session hand-off', () => {
    expect(PAGI_SHAPE.prime).toBeUndefined();
  });
});
