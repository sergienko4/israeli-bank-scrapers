/**
 * Beinleumi (FIBI) hard-model scrape shape — unit coverage for the
 ...
 * contract (userData.accounts[], accountType[0].accountType,
 * balances.currentBalance, transactions[]).
 */

import BEINLEUMI_SHAPE from '../../../../../Scrapers/Pipeline/Banks/Beinleumi/scrape/BeinleumiShape.js';
import {
  accountNumberOf,
  customerUrl,
  extractAccounts,
...
import {
  ...
  txnsUrl,
  txnsVars,
} from '../../../../../Scrapers/Pipeline/Banks/Beinleumi/scrape/BeinleumiShapeTxns.js';
import type {
  ApiBody,
...
describe('BeinleumiShape balance + identity urls', () => {
  ...
  it('customerUrl targets userData with a fresh uid on each call', () => {
    ...
    expect(first).toContain(`${BEINLEUMI_API}${USER_DATA_PATH}?uid=`);
    expect(first).not.toBe(second);
  });

  it('secondaryUrl targets the session accountType lookup', () => {
    const url = secondaryUrl();
    expect(url).toContain(`${BEINLEUMI_API}${BFF_BASE}/accountType?uid=`);
  });
...
describe('BEINLEUMI_SHAPE wiring', () => {
  it('declares GET accounts (with secondary identity) + GET balance + POST transactions', () => {
    expect(BEINLEUMI_SHAPE.customer.method).toBe('GET');
    expect(BEINLEUMI_SHAPE.customer.secondaryUrlTag).toBeDefined();
    expect(BEINLEUMI_SHAPE.balance.method).toBe('GET');
    expect(BEINLEUMI_SHAPE.transactions.method).toBe('POST');
  });

  it('carries the BeinleumiScrape step name', () => {
