/**
 * Leumi bank — Mode A integration test (Phase 11).
 *
 ...
 * committed Leumi fixture matches the DOM contract the production
 * scraper relies on, deterministically, offline, no creds.
 *
 * <p>Companion to
 * {@link ../../../Unit/Integration/Banks/Leumi/Leumi.modeB.test.ts}
 * which exercises the full pipeline chain (INIT → HOME → LOGIN →
 * AUTH-DISCOVERY → ACCOUNT-RESOLVE → DASHBOARD → SCRAPE → TERMINATE)
 * through the {@link installSimulator} state machine. Leumi is a
 * password-only declarative-login bank (`LeumiPipeline.ts` declares
 * `.withDeclarativeLogin(LEUMI_LOGIN)` with NO pre-login / OTP), so the
 * Mode B script has no OTP challenge transition.
 *
 * <p>Per-phase invariants are declared in
 * {@link ./LeumiPhaseConfig.PHASE_EXPECTATIONS}.
 */
...
import { type IPhaseExpectation, PHASE_EXPECTATIONS } from './LeumiPhaseConfig.js';
...
/**
 * Outcome of a single marker presence check. Per `no-restricted-syntax`
 * ARCHITECTURE rule, helpers return a meaningful value rather than
 * `void`; this carries the matched marker for upstream tracing in case
 * test failures need to be correlated with phase configurations.
 */
...
function assertOneMarker(html: string, marker: string, stepName: string): IMarkerCheck {
  if (!html.includes(marker)) {
    throw new ScraperError(`Leumi.modeA: step ${stepName} missing marker "${marker}"`);
  }
...
describe('Leumi Mode A — static phase drive (Phase 11)', () => {
