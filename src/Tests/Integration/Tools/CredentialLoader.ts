...
function readEnvOrThrow(
  ...
  if (raw === undefined || raw.trim() === '') {
    throw new ScraperError(
      `bank "${bankId}" missing required env var ${envVarName} (credential key "${credentialKey}")`,
    );
  }
...
function loadCredentials(bankId: string): BankCredentials {
  ...
  /**
   * Read one credential key's env-var and return the entry tuple.
   * @param key - Credential field name (drives both env lookup and output key).
   * @returns Tuple of `[credentialKey, envValue]`.
   */
  const readOne = (key: string): readonly [string, string] => {
    const envName = template[key];
    return readEnvOrThrow(key, envName, bankId);
  };
  const entries = keys.map(readOne);
