/**
 * MAX offline invalid-creds mock — captured max.co.il HTML served
 * via Playwright page.route() to verify the WK_LOGIN_RESULT failure
 * detection yields success:false WITHOUT any real network. Fixtures
 * live under C:\tmp\bank-html\max\ (gitignored); the route manifest
 * is fixtures.json alongside them.
 *
...
const FIXTURE_ROOT = 'C:/tmp/bank-html/max';
const TEST_TIMEOUT_MS = 90_000;

/** Credentials deliberately invalid — bytes never travel (zero network). */
const INVALID_CREDS = INVALID_CREDS_USERNAME_PASSWORD;

/*
 * SKIPPED — pre-existing failure tracked for PR-206-FOLLOWUP.
 * Same root cause as Discount/VisaCal: fixtures.json scaffold URL globs
 * don't match Max's real topology (www.max.co.il + /api/login/login API).
 * Re-enable after manual fixture-route authoring for Max.
 */
describe.skip('offline MAX invalid-creds', () => {
  ...
  it('scraper returns success:false when served the captured error HTML', async () => {
    if (fixtures === undefined) {
      throw new ScraperError(
        `fixtures.json missing — run "npm run capture:invalid-login -- max" or an E2E with DUMP_FIXTURES_DIR set to populate ${FIXTURE_ROOT}.`,
      );
    }
