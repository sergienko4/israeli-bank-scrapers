...
import { TransactionStatuses, TransactionTypes, type Transaction, type TransactionInstallments } from '../transactions';
...
export async function fetchAccounts(page: Page, servicesUrl: string, monthMoment: Moment): Promise<ScrapedAccount[]> {
...
export async function fetchTransactionsForMonth(opts: FetchTransactionsOpts): Promise<ScrapedAccountsWithIndex> {
...
async function enrichTxnsChunk({ page, chunk, account, opts }: EnrichChunkOpts): Promise<Transaction[]> {
...
export async function getExtraScrapAccount(opts: ExtraScrapAccountOpts): Promise<ScrapedAccountsWithIndex> {
...
export async function getAdditionalTransactionInformation(
  ...
  return runSerial(
    accountsWithIndex.map((a, i) => () => getExtraScrapAccount({ page, options, accountMap: a, month: allMonths[i] })),
  );
}

export function combineTxnsFromResults(finalResult: ScrapedAccountsWithIndex[]): Record<string, Transaction[]> {
...
function fetchMonthlyResults(opts: FetchAllOpts, allMonths: Moment[]): Promise<ScrapedAccountsWithIndex[]> {
  const { page, options, companyServiceOptions, startMoment } = opts;
  return runSerial(
    allMonths.map(
      monthMoment => () =>
        fetchTransactionsForMonth({ page, options, companyServiceOptions, startMoment, monthMoment }),
    ),
  );
}

function buildAccountsList(
  combinedTxns: Record<string, Transaction[]>,
): { accountNumber: string; txns: Transaction[] }[] {
  return Object.keys(combinedTxns).map(accountNumber => ({ accountNumber, txns: combinedTxns[accountNumber] }));
}
...
export async function fetchAllTransactions(
  ...
  return { success: true, accounts: buildAccountsList(combineTxnsFromResults(finalResult)) };
}
