...
export default class OneZeroScraper extends BaseScraper<ScraperSpecificCredentials> {
  ...
  private async fetchDeviceToken(): Promise<string> {
    const resp = await fetchPost<{ resultData: { deviceToken: string } }>(`${IDENTITY_SERVER_URL}/devices/token`, {
      extClientId: 'mobile',
      os: 'Android',
    });
  ...
  private async prepareOtp(phoneNumber: string, deviceToken: string): Promise<string> {
    const resp = await fetchPost<{ resultData: { otpContext: string } }>(`${IDENTITY_SERVER_URL}/otp/prepare`, {
  ...
  private async resolveOtpTokenViaRetriever(
    credentials: ScraperSpecificCredentials & { otpCodeRetriever: () => Promise<string>; phoneNumber: string },
  ...
  private async getIdToken(otpSmsToken: string, email: string, pass: string): Promise<string> {
    const resp = await fetchPost<{ resultData: { idToken: string } }>(`${IDENTITY_SERVER_URL}/getIdToken`, {
      otpSmsToken,
      email,
      pass,
      pinCode: '',
  ...
  private async getSessionToken(idToken: string, pass: string): Promise<string> {
    const resp = await fetchPost<{ resultData: { accessToken: string } }>(`${IDENTITY_SERVER_URL}/sessions/token`, {
      idToken,
      pass,
    });
  ...
  private async fetchMovementsPage(
    ...
    const { movements } = await fetchGraphql<{ movements: { movements: Movement[]; pagination: QueryPagination } }>(
      GRAPHQL_API_URL,
      GET_MOVEMENTS,
      {
