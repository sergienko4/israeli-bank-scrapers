...
describe('candidateToCss', () => {
  it.each<[SelectorCandidate, string]>([
    ...
    [{ kind: 'xpath', value: '//button[contains(., "כניסה")]' }, 'xpath=//button[contains(., "כניסה")]'],
...
describe('tryInContext', () => {
  ...
  it('skips a candidate that throws (cross-origin frame) and continues', async () => {
    const element = {};
    const throwThenFind = jest.fn().mockRejectedValueOnce(new Error('cross-origin')).mockResolvedValueOnce(element);
...
describe('resolveFieldContext', () => {
  ...
  it('error message includes tried candidate count and page title', async () => {
    const page = makePage({ $: jest.fn().mockResolvedValue(null) });
    await expect(resolveFieldContext(page, field, 'https://bank.test/')).rejects.toThrow(/Page title: "Bank Login"/);
  });

  it('error message includes the inspect-bank-login hint', async () => {
    const page = makePage({ $: jest.fn().mockResolvedValue(null) });
    await expect(resolveFieldContext(page, field, 'https://bank.test/')).rejects.toThrow(/inspect-bank-login\.ts/);
  });
  ...
  it('does not trigger Round 4 when called with a Frame (not a Page)', async () => {
    ...
    await expect(resolveFieldContext(frame, field, 'https://bank.test/')).rejects.toThrow(/Could not find 'username'/);
