...
describe('Selector fallback: WELL_KNOWN_SELECTORS resolution', () => {
    ...
    const scraper = new ConcreteGenericScraper(
      {
        ...
        preparePage: async page => {
          await setupRequestInterception(page, [
            { match: 'test-bank.local/login', contentType: 'text/html; charset=utf-8', body: LOGIN_HTML },
            { match: 'test-bank.local/home', contentType: 'text/html; charset=utf-8', body: HOME_HTML },
          ]);
        },
      },
      wrongIdConfig,
  ...
  it('throws a descriptive error listing all tried candidates when ALL rounds fail', async () => {
    ...
    const result = await scraper.scrape({ username: 'u', password: 'p' } as { username: string; password: string });
    ...
    const scraper = new ConcreteGenericScraper(
      {
        ...
        preparePage: async page => {
          await setupRequestInterception(page, [
            ...
            { match: 'test-bank.local/login-frame', contentType: 'text/html; charset=utf-8', body: FRAME_LOGIN_HTML },
            { match: 'test-bank.local/home', contentType: 'text/html; charset=utf-8', body: HOME_HTML },
            { match: 'test-bank.local', contentType: 'text/html; charset=utf-8', body: MAIN_PAGE_WITH_IFRAME },
          ]);
        },
      },
      iframeConfig,
