...
jest.mock('../Helpers/Navigation', () => ({
  getCurrentUrl: jest
    .fn()
    .mockResolvedValue('https://hb.unionbank.co.il/eBanking/Accounts/ExtendedActivity.aspx'),
...
beforeEach(() => {
  ...
  (getCurrentUrl as jest.Mock).mockResolvedValue(
    'https://hb.unionbank.co.il/eBanking/Accounts/ExtendedActivity.aspx',
  );
...
describe('fetchData', () => {
  it('fetches and converts transactions', async () => {
    ...
    mockTransactionTable(STANDARD_HEADERS, [
      { id: '', innerTds: ['15/06/25', 'סופר שופ', '12345', '150.00', ''] },
    ]);
  ...
  it('calculates amount as credit minus debit', async () => {
    mockTransactionTable([], []);
    mockTransactionTable(STANDARD_HEADERS, [
      { id: '', innerTds: ['15/06/25', 'Refund', '', '', '200.00'] },
    ]);
  ...
  it('handles pending transactions', async () => {
    mockTransactionTable(STANDARD_HEADERS, [
      { id: '', innerTds: ['15/06/25', 'Pending item', '', '30.00', ''] },
    ]);
    mockTransactionTable([], []);
  ...
  it('includes rawTransaction when option set', async () => {
    mockTransactionTable([], []);
    mockTransactionTable(STANDARD_HEADERS, [
      { id: '', innerTds: ['15/06/25', 'Test', '100', '50.00', ''] },
    ]);
