...
/**
 * Action exec — bind the browser-page mediator into the context.
 * @param _ctx - Sealed context (unused; input threads the same object).
 * @param input - Sealed action context threaded forward.
 * @returns Context carrying the browser-page mediator, or a failure.
 */
const BIND_EXEC: ActionExecFn = (_ctx, input) => {
  const bound = bindBrowserPageMediator(input);
