/**
 * FIBI group scrape shape — transactions helpers. A single POST to
 ...
 * `initialRequest` envelope. The captured trace returned the full window in
 * one page, so the cursor is terminal (`never`) — matching the
...
import type { IExtractPageArgs, VarsMap } from '../IApiDirectScrapeShape.js';
import { BFF_BASE, type IFibiAcct, type IFibiGroupConfig } from './FibiGroupShapeHelpers.js';
...
type FibiTxn = Record<string, unknown>;

interface ITxnsResp {
  readonly transactions?: readonly FibiTxn[];
}
 ...
 */
function buildInitialRequest(acct: IFibiAcct, ctx: IActionContext): VarsMap {
  return {
 ...
 */
export function txnsVars(acct: IFibiAcct, _cursor: false, ctx: IActionContext): VarsMap {
...
/**
 * Build a brand's transactions URL — the fixed BFF list endpoint on that
 * brand's origin (params ride the body).
 * @param cfg - Brand binding supplying the post-login origin.
 * @returns Literal transactions-list URL.
 */
export function txnsUrl(cfg: IFibiGroupConfig): WKUrlOrLiteral {
  return literalUrl(`${cfg.apiOrigin}${BFF_BASE}/list`);
}
