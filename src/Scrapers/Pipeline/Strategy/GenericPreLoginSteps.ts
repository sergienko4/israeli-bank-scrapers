 ...
 * @param mediator - Element mediator with resolver.
 * @param browserPage - Browser page (for URL wait).
 ...
 */
async function waitForFirstField(browserPage: Page): Promise<FieldReady> {
  const fieldCandidates = [...WK_LOGIN_FORM.id, ...WK_LOGIN_FORM.password];
  const locators = fieldCandidates.map((c): Locator => {
    if (c.kind === 'placeholder') return browserPage.getByPlaceholder(c.value).first();
    if (c.kind === 'labelText') return browserPage.getByLabel(c.value).first();
    return browserPage.getByText(c.value).first();
  });
  const waiters = locators.map(async (loc, i): Promise<VisibleIndex> => {
