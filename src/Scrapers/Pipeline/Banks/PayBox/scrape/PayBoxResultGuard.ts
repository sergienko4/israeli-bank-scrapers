 ...
 *
 * <p>The balance step is now `skipFetch: true` (`/sync` always 400s AND
 * poisons the session — see `PayBoxShapeHelpers.balanceVars`), so
 * `balanceDegraded` no longer fires for PayBox in practice. A rejected
 * transactions page is caught earlier and far more precisely, by
