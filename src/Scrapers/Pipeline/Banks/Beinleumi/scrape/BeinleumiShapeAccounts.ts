/**
 * Beinleumi (FIBI) scrape shape — account-identity merge across the two
 ...
 * payload marks none. Grounded in the captured trace
...
type AccountNumberDisplay = Brand<string, 'BeinleumiAccountNumberDisplay'>;
 ...
 * @returns Beinleumi account reference.
 */
function toAcct(row: IRawAccount, accountType: number): IBeinleumiAcct {
 ...
 */
export function extractAccounts(args: IExtractAccountsArgs): readonly IBeinleumiAcct[] {
  const rows = (args.body as unknown as IUserDataResp).accounts ?? [];
  const accountType = typeOf(args.secondaryBody ?? EMPTY_SECONDARY);
  return chooseAccounts(rows).map((row): IBeinleumiAcct => toAcct(row, accountType));
}
 ...
 */
export function accountNumberOf(acct: IBeinleumiAcct): AccountNumberDisplay {
...
export function customerUrl(): WKUrlOrLiteral {
  return literalUrl(`${BEINLEUMI_API}${USER_DATA_PATH}?uid=${uid()}`);
}
...
export function secondaryUrl(): WKUrlOrLiteral {
  return literalUrl(`${BEINLEUMI_API}${BFF_BASE}/accountType?uid=${uid()}`);
}
