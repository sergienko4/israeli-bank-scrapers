/**
 * Massad (FIBI group) scrape shape — account-identity merge across the two
...
type AccountNumberDisplay = Brand<string, 'MassadAccountNumberDisplay'>;
 ...
 * @returns Massad account reference.
 */
function toAcct(row: IRawAccount, accountType: number): IMassadAcct {
 ...
 */
export function extractAccounts(args: IExtractAccountsArgs): readonly IMassadAcct[] {
  const rows = (args.body as unknown as IUserDataResp).accounts ?? [];
  const accountType = typeOf(args.secondaryBody ?? EMPTY_SECONDARY);
  return chooseAccounts(rows).map((row): IMassadAcct => toAcct(row, accountType));
}
 ...
 */
export function accountNumberOf(acct: IMassadAcct): AccountNumberDisplay {
...
export function customerUrl(): WKUrlOrLiteral {
  return literalUrl(`${MASSAD_API}${USER_DATA_PATH}?uid=${uid()}`);
}
...
export function secondaryUrl(): WKUrlOrLiteral {
  return literalUrl(`${MASSAD_API}${BFF_BASE}/accountType?uid=${uid()}`);
}
