/**
 * Discount scrape shape — transactions helpers. A single date-range
 ...
 * unchanged. Split from DiscountShapeHelpers.ts to respect the 150-LOC
 * ceiling.
 *
 * The `/Date` path + query params mirror the in-repo authoritative URL
...
import { type IDiscountAcct, TITAN_API } from './DiscountShapeHelpers.js';
...
type DiscountTxn = Record<string, unknown>;

interface ITxnsResp {
  readonly CurrentAccountLastTransactions?: { readonly OperationEntry?: readonly DiscountTxn[] };
}
 ...
 */
export function txnsUrl(acct: IDiscountAcct, _cursor: false, ctx: IActionContext): WKUrlOrLiteral {
...
export function txnsExtractPage(
  args: IExtractPageArgs<IDiscountAcct, never>,
): IPage<object, never> {
