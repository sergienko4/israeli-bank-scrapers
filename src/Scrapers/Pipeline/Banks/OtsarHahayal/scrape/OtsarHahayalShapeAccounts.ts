/**
 * Otsar Hahayal (FIBI group) scrape shape — account-identity merge across
 * the two cookie-authed identity GETs, plus their urlTag builders.
 ...
 * numeric type (105 for a retail checking account) the balance path
...
type AccountNumberDisplay = Brand<string, 'OtsarHahayalAccountNumberDisplay'>;
 ...
 * @returns Otsar Hahayal account reference.
 */
function toAcct(row: IRawAccount, accountType: number): IOtsarHahayalAcct {
 ...
 */
export function extractAccounts(args: IExtractAccountsArgs): readonly IOtsarHahayalAcct[] {
  const rows = (args.body as unknown as IUserDataResp).accounts ?? [];
  const accountType = typeOf(args.secondaryBody ?? EMPTY_SECONDARY);
  return chooseAccounts(rows).map((row): IOtsarHahayalAcct => toAcct(row, accountType));
}
 ...
 */
export function accountNumberOf(acct: IOtsarHahayalAcct): AccountNumberDisplay {
...
export function customerUrl(): WKUrlOrLiteral {
  return literalUrl(`${OTSAR_HAHAYAL_API}${USER_DATA_PATH}?uid=${uid()}`);
}
...
export function secondaryUrl(): WKUrlOrLiteral {
  return literalUrl(`${OTSAR_HAHAYAL_API}${BFF_BASE}/accountType?uid=${uid()}`);
}
