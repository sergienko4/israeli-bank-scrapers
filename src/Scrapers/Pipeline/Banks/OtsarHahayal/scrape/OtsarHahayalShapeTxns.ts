/**
 * Otsar Hahayal (FIBI group) scrape shape — transactions helpers. A single
 * POST to `bff-balancetransactions/api/v1/transactions/list` with the
 * account number (numeric), account type, branch, and the scrape date
 * window in an `initialRequest` envelope. The captured trace returned the
 * full window in one page, so the cursor is terminal (`never`) — matching
 * the Hapoalim/Leumi single-page precedent. Raw rows flow downstream to
 * the field-mapping Data Mapper unchanged (amount = creditAmount −
 * debitAmount, date from dateOfRegistration).
 *
 * Contract shared with Beinleumi (same FIBI Mataf portal); cloned per the
 * zero-cross-bank-import convention. Split from OtsarHahayalShapeHelpers.ts
 * to respect the 150-LOC ceiling.
 */
...
import { BFF_BASE, type IOtsarHahayalAcct, OTSAR_HAHAYAL_API } from './OtsarHahayalShapeHelpers.js';
...
type OtsarHahayalTxn = Record<string, unknown>;

interface ITxnsResp {
  readonly transactions?: readonly OtsarHahayalTxn[];
}
 ...
 */
function buildInitialRequest(acct: IOtsarHahayalAcct, ctx: IActionContext): VarsMap {
  return {
...
export function txnsUrl(): WKUrlOrLiteral {
  return literalUrl(`${OTSAR_HAHAYAL_API}${BFF_BASE}/list`);
}
 ...
 */
export function txnsVars(acct: IOtsarHahayalAcct, _cursor: false, ctx: IActionContext): VarsMap {
...
export function txnsExtractPage(
  args: IExtractPageArgs<IOtsarHahayalAcct, never>,
): IPage<object, never> {
