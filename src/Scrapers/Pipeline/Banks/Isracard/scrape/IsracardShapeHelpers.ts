/**
 * Isracard scrape shape — card-list extractor + the customer POST body/URL
 * for the DigitalV3 transactions API. Transactions helpers live in
 * IsracardShapeTxns.ts; response row-merging in IsracardShapeExtract.ts.
 ...
 *
 * Grounded in the Isracard network trace: customer =
 ...
 * companyCode}. Isracard shares the base-isracard-amex DigitalV3 backbone
 * with Amex; only the SPA host differs (web.isracard.co.il). Raw transaction
 * rows normalise downstream via PIPELINE_WELL_KNOWN_TXN_FIELDS (the Data
...
export const ISRACARD_API = 'https://web.isracard.co.il';
...
type CardNumberDisplay = Brand<string, 'IsracardCardNumberDisplay'>;

/**
 * Isracard card reference. `cardSuffix` (last-4) is sent as the per-card
 * transactions `card4Number`; `companyCode` selects the issuing brand
 * (e.g. 11) in the transactions body — distinct from the fixed "99"
 * GetCardList query code.
 */
export interface IIsracardCard {
 ...
 */
function toCard(c: IRawCard): IIsracardCard {
 ...
 */
export function extractCards(args: IExtractAccountsArgs): readonly IIsracardCard[] {
 ...
 */
export function accountNumberOf(card: IIsracardCard): CardNumberDisplay {
 ...
 * @returns Literal Isracard GetCardList URL.
 */
