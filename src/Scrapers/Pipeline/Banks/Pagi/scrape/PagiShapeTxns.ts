/**
 * Pagi (FIBI group) scrape shape — transactions helpers. A single POST to
 ...
 * field-mapping Data Mapper unchanged (amount = creditAmount − debitAmount,
 * date from dateOfRegistration).
 *
 * Contract shared with Beinleumi (same FIBI Mataf portal); cloned per the
 * zero-cross-bank-import convention. Split from PagiShapeHelpers.ts to
...
import { BFF_BASE, type IPagiAcct, PAGI_API } from './PagiShapeHelpers.js';
...
type PagiTxn = Record<string, unknown>;

interface ITxnsResp {
  readonly transactions?: readonly PagiTxn[];
}
 ...
 */
function buildInitialRequest(acct: IPagiAcct, ctx: IActionContext): VarsMap {
  return {
...
export function txnsUrl(): WKUrlOrLiteral {
  return literalUrl(`${PAGI_API}${BFF_BASE}/list`);
}
 ...
 */
export function txnsVars(acct: IPagiAcct, _cursor: false, ctx: IActionContext): VarsMap {
 ...
 */
export function txnsExtractPage(args: IExtractPageArgs<IPagiAcct, never>): IPage<object, never> {
