...
/** Number of random bytes generated by the 'random-hex-16' bootstrap (16 hex chars). */
const BOOTSTRAP_HEX16_BYTES = 8;

/**
 * Run a named bootstrap generator. Each kind in {@link SeedCarryBootstrapKind}
 * has a closed implementation owned by the mediator so the config stays
 * data-only. Currently only `'random-hex-16'` is supported (8 random
 * bytes rendered as 16 lowercase hex chars — the device-id shape
 * banks use for first-cold-run install identification). When a
 * second kind is added, restore the discriminator
 * parameter + dispatch here.
 * @returns Generated string value (mediator-defined per kind).
 */
function runSeedBootstrap(): string {
  return randomBytes(BOOTSTRAP_HEX16_BYTES).toString('hex');
}

/** Normalised seed spec — uniform shape after resolving the string-or-object union. */
interface INormalisedSeedSpec {
  readonly field: string;
  readonly bootstrap?: SeedCarryBootstrapKind;
}

/**
 * Normalise one entry from `seedCarryFromCreds` (string shorthand or
 * {@link ISeedCarrySource} object) into the uniform shape.
 * @param spec - Source spec.
 * @returns Normalised spec.
 */
function normaliseSeedSpec(spec: string | ISeedCarrySource): INormalisedSeedSpec {
  if (typeof spec === 'string') return { field: spec };
  return { field: spec.field, bootstrap: spec.bootstrap };
}

/**
 * Read the seed value for one spec from creds, falling back to the
 * declared bootstrap when creds is missing/empty. Fails when neither
 * a creds value nor a bootstrap is available.
 * @param creds - Caller credentials.
 * @param spec - Normalised seed spec.
 * @returns Procedure with the resolved string value.
 */
function readSeedValue(
  creds: Readonly<Record<string, unknown>>,
  spec: INormalisedSeedSpec,
): Procedure<string> {
  const value = creds[spec.field];
  if (typeof value === 'string' && value.length > 0) return succeed(value);
  if (spec.bootstrap !== undefined) {
    const generated = runSeedBootstrap();
    return succeed(generated);
  }
  return fail(
    ScraperErrorTypes.Generic,
    `seedCarryFromCreds: creds.${spec.field} missing or non-string`,
  );
}

/** Reducer ctx for {@link reduceSeedField}. */
interface IReduceSeedFieldCtx {
  readonly spec: INormalisedSeedSpec;
  readonly creds: Readonly<Record<string, unknown>>;
}
...
function reduceSeedField(
  ...
  if (!isOk(acc)) return acc;
  const valProc = readSeedValue(ctx.creds, ctx.spec);
  if (!isOk(valProc)) return valProc;
  return succeed({ ...acc.value, [ctx.spec.field]: valProc.value });
}

/**
 * Mirror each declared seed source into carry as a string. Fails fast
 * when a string-only spec is missing on creds and no bootstrap is
 * configured. Bootstrap-equipped specs auto-generate the first
 * value (e.g. a fresh device identifier on cold run).
 * @param carry - Current carry record.
 * @param specs - Seed source declarations (string shorthand or object).
 * @param creds - Caller credentials.
 * @returns Procedure with the seeded carry.
 */
function applySeedCarryFromCreds(
  carry: Record<string, JsonValue>,
  specs: readonly (string | ISeedCarrySource)[],
  creds: Readonly<Record<string, unknown>>,
): Procedure<Record<string, JsonValue>> {
  const seed: Procedure<Record<string, JsonValue>> = succeed({ ...carry });
  return specs.reduce<Procedure<Record<string, JsonValue>>>(
    (acc, raw) => reduceSeedField(acc, { spec: normaliseSeedSpec(raw), creds }),
    seed,
  );
}
